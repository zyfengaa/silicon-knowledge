# 模块 10：其他加速器（Other Accelerators）-- 练习题

## 问题（Questions）

### 问题 1：FPGA 设计理念（FPGA Design Philosophy）
...
### 问题 2：FPGA vs. GPU -- 场景选择（Scenario Selection）
...
### 问题 3：晶圆级 vs. 分布式（Wafer-Scale vs. Distributed）
...
### 问题 4：近内存计算动机（Near-Memory Computing Motivation）
...
a) 请分析这个操作在传统冯诺依曼架构（Von Neumann architecture）中的瓶颈。为什么它是"memory-bound"的？
b) 解释 Samsung HBM-PIM 如何加速这种操作。具体是哪个硬件环节被改善了？
c) 假设一个程序的计算密集度远高于访存密集度（如大型矩阵乘法），PIM 是否能带来收益？为什么？
...
- CPU 每周期有大约 5-15% 功耗用于取指/译码，额外的功耗用于 OOO（out-of-order execution / 乱序执行）、分支预测、缓存一致性
...
### 答案 2：FPGA vs. GPU -- 场景选择

**a) 100 Gbps 网络包处理 → FPGA**

原因：线速处理要求每个包在纳秒级完成处理。GPU 的 kernel 启动延迟（约 10μs）和批处理模式不适合这种实时流处理。FPGA 可以创建全流水线的包处理管道，延迟仅数百纳秒。
...
原因：端到端延迟需求 < 5μs 排除了 GPU（kernel 启动延迟约 10-50μs）。FPGA 可以将推理模型实现为固定延迟的流水线，延迟可稳定在 1-2μs 以内。
...
1. **单芯片算力上限**：约 400 TFLOPS，远低于大型 GPU 集群（上万 GPU 可达 EFLOPS）
...
SAXPY（y = a·x + y）的算术强度（arithmetic intensity）为：
...
改善的关键环节:
1. 消除数据搬运能耗（约 180 pJ/word 的 DRAM 访问能耗节省）
2. 释放片外总线带宽
